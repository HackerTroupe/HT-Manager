...
def register_addctfmember_command(bot: Bot) -> None:
    @bot.tree.command(name="addctfmember", description="Manually add a participant to a CTF")
    @admin_only()
    @discord.app_commands.describe(ctf_id="ID of the CTF", member="Member to add")
    async def addctfmember(
        ...
        await interaction.response.send_message(f"Added {member.mention} to CTF #{ctf_id}.")
